...
import {
    Loader2, CheckCircle2, QrCode, ExternalLink,
    RefreshCcw, Clock, ShieldCheck, CreditCard
} from 'lucide-react';
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center p-12 space-y-4 bg-slate-900 border border-white/10 rounded-3xl">
                ...
                <p className="text-slate-400 font-medium">QPay QR код үүсгэж байна...</p>
            </div>
        );
    }
        ...
        return (
            ...
            >
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-slate-400">Таны захиалга баталгаажлаа. Түр хүлээнэ үү...</p>
                </div>
            </motion.div>
        );
    }
        ...
        <div className="bg-slate-900 border border-white/10 rounded-3xl overflow-hidden shadow-2xl max-w-md mx-auto">
            ...
            <div className="p-6 border-b border-white/5 bg-slate-800/50 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-950 rounded-full border border-white/5">
                    ...
                    <span className="text-xs font-mono font-bold text-slate-300">{formatTime(timeLeft)}</span>
                </div>
            </div>
            ...
            <div className="p-8 space-y-8">
                    ...
                    <AnimatePresence>
                        {checking && (
                            ...
                            >
                                ...
                                <span className="text-[10px] font-bold text-slate-900 uppercase tracking-widest">Шалгаж байна</span>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
                ...
                <div className="text-center space-y-1">
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Төлөх дүн</p>
                    <p className="text-3xl font-black text-white">{amount.toLocaleString()}₮</p>
                </div>

                {/* Banking App Links */}
                <div className="space-y-4">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest text-center">Банкны апп-аар нээх</p>
                    <div className="grid grid-cols-4 gap-3">
                        {qpayData?.urls.slice(0, 8).map((app, i) => (
                            ...
                            >
                                ...
                                <span className="text-[10px] text-slate-500 font-medium truncate w-full text-center">{app.name}</span>
                            </a>
                        ))}
                    </div>
                </div>
